document.addEventListener("DOMContentLoaded", function () {
  ...
  let circuitList1 = [];
  let circuitList2 = [];
  // Regroupena ny circuitList1 sy circuitList2 anaty variable iray circuitList
  let circuitList = [circuitList1, circuitList2];

  /******************************************************************************** */
  /**************************MDESSINNE ANLE CAHIER AMN CANVAS********************** */
  function drawGrid(cellSize) {
    ...
    const canvasHeight = Math.min(
      canvas.height + 5,
      gameContainer.clientHeight - 150
    );
    ...
    for (
      let y = -cellSize / 2 + _offsetY;
      y <= canvas.height - coeffY * _offsetY;
      y += cellSize
    ) {
      let distance = cellSize / 4;
      for (let i = 1; i <= 3; i++) {
        ...
        ctx.strokeStyle = "#4455C9CC";
        ctx.lineWidth = 0.3 * lineScale;
        ctx.stroke();
      }
    }
    ...
    //lignes verticales
    for (
      let x = cellSize / 2 + _offsetX;
      x <= canvas.width - coeffX * _offsetX;
      x += cellSize
    ) {
      ctx.beginPath();
      ctx.moveTo(x, 0);
  ...
  function drawPlayerPoints(points, color) {
    ...
    for (let point of points) {
      ctx.beginPath();
      ctx.arc(
        point.x * cellSize + cellSize / 2 + _offsetX,
        point.y * cellSize + cellSize / 2 + _offsetY,
        cellSize * 0.105,
        0,
        2 * Math.PI
      );
  ...
  function updateTimer() {
    if (!timerPaused && tD != 99) {
      ...
      const currentTime = Math.max(
        ...
        timePaused +
          timerDuration -
          Math.floor((Date.now() - turnStartGame) / 1000)
      );
      const minutes = Math.floor(currentTime / 60);
      const seconds = currentTime % 60;

      currentTurnElement.textContent = `Temps restant: ${minutes}:${
        seconds < 10 ? "0" : ""
      }${seconds}`;
  ...
  function updateReflectionTime() {
    ...
    if (!timerPaused && tR != 0) {
      const turnDurationSeconds = tR; // TEMPS DE REFLEXUION
      ...
      remainingTimeSeconds =
        turnDurationSeconds - currentTimeSeconds + timePaused;
  ...
  // Appel fonction de mise à jour du minuteur chaque seconde
  setInterval(updateReflectionTime, 1000);

  // Appel fonction de mise à jour du minuteur immédiatement pour l'initialiser
  updateReflectionTime();

  // Affichage des coordonnées de la souris// DEBOGAGE***********************************************
  ...
  function validateCircuit(circuit, circuitList) {
    for (let i = circuitList[currentPlayer - 1].length - 1; i >= 0; i--) {
      let palindromeCheck = isPalindrome(
        circuit,
        circuitList[currentPlayer - 1][i]
      );
      if (palindromeCheck) {
        return false;
      }
  ...
  function reliable(x, y, leng) {
    if (grid[y][x] === currentPlayer) {
      for (let i = leng; i > 0; i--) {
        if (
          x === xyList[currentPlayer - 1][i][0] &&
          y === xyList[currentPlayer - 1][i][1]
        ) {
          return false;
        }
      }
          ...
          if (x + j === debX && y + i === debY) {
            ...
            let _apart =
              j === -1 && i === -1
                ...
                ? 9
                : apart;
            let recursiveBuildUp = buildUpCircuit(
              x + j,
              y + i,
              xyList,
              circuitList,
  ...
  function updateCurrentTurn() {
    ...
    gameContainer.style.backgroundColor =
      currentPlayer === 1 ? player1Color + "BF" : player2Color + "BF";
  }
  ...
  function redraw(cellSize) {
    ...
    updateCurrentTurn();
    drawClosedCircuit();
    // displayMatrix();  DEBOGAGE********************************************************
  }

  // Fonction pour afficher la matrice DEBOGAGE****************************************************
